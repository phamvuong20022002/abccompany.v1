...
let url_displayTable = BASE_URL + "/staff/statistcisallpartner/" + MANV
let url_search = BASE_URL + "/staff/statisticspartner"
let dataWidgets
...
async function statisticsButton(type){
    ...
    await fetch(url_search, {
        ...
        if(data.length === 0) {
            swal({
                title: "ANNOUNCE!",
                text: "Không có dữ liệu về doanh số bán hàng của đối tác " + dataReq.madt +" trong thời gian tìm kiếm!" ,
                icon: "info",
                button: "Click me!"
            })
            return
        ...
        if (keys[0] === "ERROR") {
            ...
            if(type === "ALL"){
                ...
                chartname.innerHTML = "Đồ Thị Doanh Số Bán Hàng Theo Các Năm Từ " + chartLabels[0] + " Đến Năm " + chartLabels[chartLabels.length - 1] + " Của Đối Tác " + dataReq.madt
            }
            else if(type === "YEAR"){
                chartname.innerHTML = "Đồ Thị Doanh Số Bán Hàng Theo Năm Được Tìm Kiếm Của Đối Tác " + dataReq.madt
            }
            else if(type === "MONTH"){
                chartname.innerHTML = "Đồ Thị Doanh Số Bán Hàng Theo Tháng Của Năm Được Tìm Kiếm Của Đối Tác " + dataReq.madt
            }
            else {
                chartname.innerHTML ="Đồ Thị Doanh Số Bán Hàng Trong Ngày " + dataReq.date + " Của Đối Tác " + dataReq.madt
            }

            let chartName = "Doanh Số Đối tác " + dataReq.madt
            //display chart
        ...
        for(let i = 0; i < data.length; i++){
            ...
            for(let i = 0; i < keys.length; i++){
                ...
                if(i === 0){
                    ...
                    a.href = "#"
                    a.onclick = () => {
                        localStorage.setItem("onePartnerCode",oneData[keys[0]])
...
function DisplayWidgets(data){
    ...
    // display
    let numofPartners = data.length
    let sumRevenue = 0;
    let commissions_discount = 0    
    for(let i = 0; i < data.length; i++){
        sumRevenue += parseInt(data[i].DOANHTHU)
        commissions_discount += (parseInt(data[i].HHTHANG) + parseInt(data[i].HHDH))
    }
    const revenue = document.getElementById("revenue") // Revenue
    revenue.innerHTML = formatNumber(sumRevenue)
    
    const commissions = document.getElementById("commissions") // commissions
    commissions.innerHTML = formatNumber(commissions_discount)

    const numPartners = document.getElementById("numPartners") // number of Partners
    numPartners.setAttribute("class","count")
    numPartners.innerHTML = numofPartners
